...
import legacy_b06_direct_v556 as direct
from framework.backtest.robust_covariance import robust_covariance
from legacy_b06_direct_v556 import (
    bounded_simplex_v556,
    direct_active_alpha_v556,
    legacy_b06_target_v556,
)
...
def test_bounded_simplex_is_the_exact_box_constrained_projection():
    ...
    actual = bounded_simplex_v556(values, LOWER, UPPER)
    ...
    assert np.allclose(
        bounded_simplex_v556(actual, LOWER, UPPER), actual, atol=1.0e-12
    )

    with pytest.raises(ValueError):
        bounded_simplex_v556(values, np.full(4, 0.30), UPPER)
...
def test_stability_sleeve_is_bounded_inverse_volatility_and_exactly_blended():
    ...
    expected_sleeve = bounded_simplex_v556(inverse_volatility, LOWER, UPPER)
...
def test_robust_covariances_are_causal_12_and_36_month_estimates_and_set_kappa():
    ...
    expected_target = bounded_simplex_v556(
...
def test_annual_volatility_matches_annualized_covariance_diagonal_without_second_annualization():
    _, diagnostics = _target(_history())
    covariance_12 = np.asarray(diagnostics["robust_covariance_12m"], dtype=float)
    annual_volatility = np.asarray(diagnostics["annual_volatility"], dtype=float)
    expected = np.maximum(np.sqrt(np.diag(covariance_12)), 0.02)

    assert np.allclose(annual_volatility, expected, rtol=0.0, atol=1.0e-14)
    above_floor = annual_volatility > 0.02 + 1.0e-14
    assert np.allclose(
        annual_volatility[above_floor] ** 2,
        np.diag(covariance_12)[above_floor],
        rtol=1.0e-13,
        atol=1.0e-15,
    )
    source = inspect.getsource(direct).replace(" ", "").lower()
    assert "diag(covariance12)*12" not in source
    assert "diag(covariance12)*12.0" not in source


def test_frozen_signal_spec_is_json_native_and_roundtrips_exactly():
    spec = direct.frozen_spec_v556()
    encoded = direct.json.dumps(spec, sort_keys=True, separators=(",", ":"), allow_nan=False)
    assert direct.json.loads(encoded) == spec
    assert isinstance(spec["asset_order"], list)
    assert isinstance(spec["horizons"], list)
    assert isinstance(spec["horizon_weights"], list)
