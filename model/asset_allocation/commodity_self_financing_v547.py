...
OUTPUT_SCHEMA_V547 = "asset-allocation-panel-v547-causal-self-financing-d2-research/1.0"
INDEX_START_V547 = "2014-12-24"
...
def construct_panel_v547(
    ...
    *,
    ...
    index_start: str = INDEX_START_V547,
) -> dict[str, Any]:
    ...
    for root, rows in dominant.items():
        for contract in rows.values():
            if base._root_from_contract(contract) != root:
                raise ValueError("v547_contract_root_mapping_invalid")
            existing = contract_to_root.setdefault(contract, root)
            if existing != root:
                raise ValueError("v547_contract_root_collision")
    prices = {
    ...
    if index_start not in calendar:
        raise ValueError("v547_index_start_not_trading_day")
    ...
    if not collateral_rows or not all(math.isfinite(value) and value >= 0.0 for _, value in collateral_rows):
        raise ValueError("v547_collateral_invalid")
    collateral_index = 0
    ...
    for when in calendar:
        ...
        for root, contract in current_contracts.items():
            ...
            if row is None:
                raise ValueError(f"v547_dominant_price_missing:{root}:{when}")
        ...
        if previous_date is None:
            raise ValueError("v547_previous_execution_date_missing")
        ...
        if month_changed:
            ...
            for sector_roots in base.SECTORS_V543.values():
                ...
                if set(sleeve) != set(sector_roots):
                    raise ValueError("v547_sector_root_silently_dropped")
            ...
            if not math.isclose(sum(new_target.values()), 1.0, abs_tol=1.0e-12):
                raise ValueError("v547_target_notional_not_one")
            target = new_target
        ...
        for root, weight in start_root_exposure.items():
            if not math.isfinite(weight) or weight <= 0.0:
                raise ValueError(f"v547_root_weight_nonpositive:{root}:{when}")
        ...
        for contract in sorted(set(previous_exposure) | set(new_exposure)):
            ...
            if not base._fee_is_usable(fee) or multiplier is None:
                raise ValueError(f"v547_previous_day_trade_input_missing:{contract}:{previous_date}")
        ...
        if last_prior_rate is None:
            raise ValueError(f"v547_prior_collateral_missing:{previous_date}")
        day_count = (date.fromisoformat(when) - date.fromisoformat(previous_date)).days
        if day_count <= 0:
            raise ValueError("v547_calendar_not_increasing")
        ...
        if not math.isfinite(daily_return) or daily_return <= -1.0:
            raise ValueError(f"v547_daily_return_invalid:{when}")
        ...
        if not all(
            ...
            raise ValueError(f"v547_end_exposure_invalid:{when}")
        end_exposure = {
    ...
    if len(daily_ledger) < 500:
        raise ValueError("v547_commodity_ledger_too_short")
    if any(
        row["is_monthly_rebalance"]
        and abs(float(row["rebalance_target_notional"]) - 1.0) > 1.0e-12
        for row in daily_ledger
    ):
        raise ValueError("v547_monthly_target_notional_incomplete")
    ...
    if len(common) < 61:
        raise ValueError("v547_common_months_insufficient")
    months = common[1:]
    returns = []
    levels = []
    for index in range(1, len(common)):
        ...
        if not all(math.isfinite(value) and value > -1.0 for value in row):
            raise ValueError(f"v547_monthly_return_invalid:{right}")
        returns.append(row)
        levels.append(right_values)
    output: dict[str, Any] = {
        "schema_version": OUTPUT_SCHEMA_V547,
        ...
        "commodity": {
            "method": "real_contract_rule0_T_minus_1_monthly_rebalanced_self_financing",
...
def main() -> int:
    ...
    result = construct_panel_v547(source, trading)
...
__all__ = ["OUTPUT_SCHEMA_V547", "construct_panel_v547"]
