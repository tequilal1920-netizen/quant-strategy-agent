"""Governed long-sample v5.5.1 research with physical test isolation.
...
SCHEMA_V554 = "asset-allocation-v554-T2-signal-self-financing-act360-long/1.0"
PANEL_SCHEMA_V554 = "asset-allocation-panel-v553-T2-signal-self-financing-act360-d2-research/1.0"
CANONICAL_SOURCE_HASH_V554 = "E0E7001141EED0C8D1A46E58F47C875ADBC628BF62B491773C5A8BBF71D4F731"
CANONICAL_TRADING_HASH_V554 = "7D103E6EFB4923C34BA95DAD4B1A1E7F767CB41E88C697E6768938C4CA33436C"
LOOKBACK_V554 = 36
TRAIN_MONTHS_V554 = tuple(f"{year:04d}{month:02d}" for year in (2018, 2019) for month in range(1, 13))
VALIDATION_MONTHS_V554 = tuple(f"{year:04d}{month:02d}" for year in (2020, 2021) for month in range(1, 13))
PRETEST_MONTHS_V554 = TRAIN_MONTHS_V554 + VALIDATION_MONTHS_V554
PRETEST_YEARS_V554 = ("2018", "2019", "2020", "2021")
TEST_START_V554 = "202201"
...
def candidate_grid_v554() -> tuple[dict[str, Any], ...]:
    rows = []
    for index, (view_scale, uncertainty) in enumerate(
        ...
        rows.append({"id": f"V554-REL-{index:02d}", "mode": "benchmark_relative", "parameters": asdict(parameters)})
    for index, (view_scale, anchor_penalty) in enumerate(
        ...
        rows.append({"id": f"V554-ABS-{index:02d}", "mode": "absolute_no_benchmark", "parameters": asdict(parameters)})
    return tuple(rows)


GRID_HASH_V554 = _canonical_hash(candidate_grid_v554())


def _month_number(value: str) -> int:
    text = str(value)
    if len(text) != 6 or not text.isdigit():
        raise ValueError("v554_month_must_be_YYYYMM")
...
def _validate_panel(panel: Mapping[str, Any], *, allow_test: bool) -> tuple[list[str], np.ndarray]:
    if panel.get("schema_version") != PANEL_SCHEMA_V554:
        raise ValueError("v554_panel_schema_invalid")
    if tuple(panel.get("asset_order") or ()) != ASSET_ORDER_V541:
        raise ValueError("v554_panel_asset_order_invalid")
    if panel.get("deployment_allowed") is not False or (panel.get("data_quality") or {}).get("status") != "D2_research_not_D3":
        raise ValueError("v554_panel_governance_boundary_invalid")
    lineage = panel.get("source_lineage") or {}
    if (
        lineage.get("source_content_sha256") != CANONICAL_SOURCE_HASH_V554
        or lineage.get("trading_parameters_content_sha256") != CANONICAL_TRADING_HASH_V554
        or lineage.get("commodity_builder") != "commodity_self_financing_v553"
        or lineage.get("collateral_source_method") != "get_interbank_offered_rate.Shibor_ON_fallback"
        or lineage.get("collateral_day_count") != "ACT/360"
    ):
        raise ValueError("v554_panel_source_lineage_invalid")
    body = dict(panel)
    stored_hash = str(body.pop("content_sha256", ""))
    if stored_hash != _canonical_hash(body):
        raise ValueError("v554_panel_content_hash_mismatch")
    ...
    if len(set(numbers)) != len(numbers) or any(right - left != 1 for left, right in zip(numbers, numbers[1:])):
        raise ValueError("v554_panel_months_not_unique_contiguous")
    ...
    if returns.shape != (len(months), 4) or not np.all(np.isfinite(returns)) or np.any(returns <= -1.0):
        raise ValueError("v554_panel_returns_invalid")
    if not set(PRETEST_MONTHS_V554).issubset(months) or len(months) < LOOKBACK_V554 + len(PRETEST_MONTHS_V554):
        raise ValueError("v554_panel_required_protocol_months_missing")
    if not allow_test and any(month >= TEST_START_V554 for month in months):
        raise ValueError("v554_selector_simulator_received_test_month")
...
def _strip_runtime_fields(value: Any) -> Any:
    ...
    if isinstance(value, float):
        if not math.isfinite(value):
            raise ValueError("v554_nonfinite_output")
        return value
    return value
...
def _pretest_panel(panel: Mapping[str, Any]) -> dict[str, Any]:
    months = [str(item) for item in panel["months"]]
    keep = [index for index, month in enumerate(months) if month < TEST_START_V554]
    output = {
        key: deepcopy(value)
        for key, value in panel.items()
        if key not in {"months", "returns", "levels", "content_sha256", "commodity"}
    }
    commodity = deepcopy(panel.get("commodity") or {})
    commodity["daily_ledger"] = [
        row for row in commodity.get("daily_ledger", [])
        if str(row.get("date", ""))[:7].replace("-", "") < TEST_START_V554
    ]
    commodity["monthly_nav"] = {
        month: value for month, value in (commodity.get("monthly_nav") or {}).items()
        if str(month) < TEST_START_V554
    }
...
def _sample(month: str) -> str:
    if month in TRAIN_MONTHS_V554:
        return "train"
    if month in VALIDATION_MONTHS_V554:
        return "validation"
...
def _allocate(
    ...
    macro_levels = np.zeros((LOOKBACK_V554, 4))
    macro_admission = np.zeros((LOOKBACK_V554, 4), dtype=bool)
    arguments = (
        window,
        macro_levels,
        macro_admission,
...
def _simulate_v554(panel: Mapping[str, Any], spec: Mapping[str, Any], *, allow_test: bool) -> dict[str, Any]:
    ...
    weight_rows = []
    maximum_kkt = 0.0
    for signal_index in range(LOOKBACK_V554 - 1, len(returns) - 1):
        left = signal_index - LOOKBACK_V554 + 1
        ...
        if optimizer["status"] != "optimal" or optimizer["solver"]["fallback_used"]:
            raise RuntimeError("v554_monthly_solver_failed")
        kkt = float(optimizer["solver"]["maximum_kkt_residual"])
        if not math.isfinite(kkt) or kkt > 1.0e-7:
            raise RuntimeError("v554_monthly_kkt_failed")
    ...
    return {
        ...
        "pretest_calendar_years": {
            ...
            for year in PRETEST_YEARS_V554
        },
        "returns": rows,
        "weights": weight_rows,
...
def _number(row: Mapping[str, Any], key: str) -> float:
    value = row.get(key)
    if value is None:
        raise ValueError(f"v554_required_metric_missing:{key}")
    number = float(value)
    if not math.isfinite(number):
        raise ValueError(f"v554_required_metric_nonfinite:{key}")
    return number


def select_pretest_v554(
    candidates: Sequence[Mapping[str, Any]], mode: str
) -> tuple[str | None, list[dict[str, Any]]]:
    if mode not in {"benchmark_relative", "absolute_no_benchmark"}:
        raise ValueError("v554_mode_invalid")
    board = []
    for candidate in candidates:
        if candidate["spec"]["mode"] != mode:
            raise ValueError("v554_selector_mode_mismatch")
        if set(candidate.get("metrics") or {}) != {"train", "validation"}:
            raise ValueError("v554_selector_received_non_pretest_metrics")
        years = candidate.get("pretest_calendar_years") or {}
        if tuple(sorted(years)) != PRETEST_YEARS_V554 or any(int(years[year].get("months") or 0) != 12 for year in PRETEST_YEARS_V554):
            raise ValueError("v554_selector_calendar_boundary_invalid")
        ...
        if int(train.get("months") or 0) != 24 or int(validation.get("months") or 0) != 24:
            raise ValueError("v554_selector_split_length_invalid")
        yearly = [years[year] for year in PRETEST_YEARS_V554]
...
def _current_target(panel: Mapping[str, Any], spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    full = _simulate_v554(panel, spec, allow_test=True)
    previous = np.asarray(full["end_drifted_weights"], dtype=float)
    left = len(returns) - LOOKBACK_V554
    diagnostics = _allocate(
        str(spec["mode"]),
        returns[left:],
        months[left:],
...
def build_research_v554(panel: Mapping[str, Any]) -> dict[str, Any]:
    _validate_panel(panel, allow_test=True)
    pretest = _pretest_panel(panel)
    _validate_panel(pretest, allow_test=False)
    pretest_results = [_simulate_v554(pretest, spec, allow_test=False) for spec in candidate_grid_v554()]
    ...
    for mode in ("benchmark_relative", "absolute_no_benchmark"):
        ...
        selected[mode], boards[mode] = select_pretest_v554(
            [_selection_payload(row) for row in mode_results], mode
        )
        if selected[mode] is None:
            reports[mode] = None
        else:
            spec = next(row for row in candidate_grid_v554() if row["id"] == selected[mode])
            reports[mode] = _test_report(panel, spec)
    output: dict[str, Any] = {
        "schema_version": SCHEMA_V554,
        ...
        "candidate_grid": list(candidate_grid_v554()),
        "candidate_grid_sha256": GRID_HASH_V554,
...
def main() -> int:
    ...
    result = build_research_v554(panel)
