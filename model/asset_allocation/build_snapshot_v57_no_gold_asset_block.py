"""Build the v5.7 no-gold asset-allocation page snapshot.
...
* policy anchor: original 60/15/15/10 with gold removed and the remaining
  assets re-normalised to 66.67/16.67/16.67;
...
SCHEMA_V57 = "5.7.0"
ENGINE_V57 = "asset-allocation-v57-three-asset-no-gold-cycle-framework"
...
# User's former 60/15/15/10 benchmark after deleting gold and re-normalising the
# remaining three assets.  This avoids quietly assigning the old gold sleeve to
# a single asset.
POLICY = np.array([2.0 / 3.0, 1.0 / 6.0, 1.0 / 6.0], dtype=float)
DISPLAY_EQUAL = np.array([1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0], dtype=float)
...
def _validate_panel(panel: Mapping[str, Any]) -> None:
    if tuple(panel.get("asset_order") or ()) != SOURCE_ASSET_ORDER:
        raise ValueError("v57_source_panel_asset_order_mismatch")
    if panel.get("content_sha256") != EXPECTED_PANEL_HASH:
        raise ValueError("v57_source_panel_hash_mismatch")
    if (panel.get("data_quality") or {}).get("production_ready") is not False:
        raise ValueError("v57_panel_must_remain_research_only")
...
def _select_three_asset_returns(panel: Mapping[str, Any]) -> tuple[list[str], np.ndarray]:
    ...
    if raw.ndim != 2 or raw.shape[1] != len(SOURCE_ASSET_ORDER):
        raise ValueError("v57_panel_returns_shape_invalid")
    selected = raw[:, SOURCE_INDICES]
    if not np.all(np.isfinite(selected)):
        raise ValueError("v57_three_asset_returns_non_finite")
...
def _rolling_erc_rows(months: Sequence[str], returns: np.ndarray) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for signal_index in range(35, len(returns) - 1):
        ...
        if erc.status != "optimal":
            raise RuntimeError(f"v57_erc_failed:{months[signal_index]}")
...
def _bl_rows(months: Sequence[str], returns: np.ndarray) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for signal_index in range(35, len(returns) - 1):
        ...
        relative_view_alpha = 0.0025 * np.asarray(
        ...
        solved = optimize_relative_v539(
            ...
            active_l2_penalty=0.05,
        )
        if solved.get("status") != "optimal":
            raise RuntimeError(f"v57_bl_optimizer_failed:{months[signal_index]}:{solved.get('status')}")
...
def build_snapshot() -> dict[str, Any]:
    ...
    strategies = {
        "black_litterman": _strategy_payload(
            ...
            construction=[
                ...
                "政策锚由60/15/15/10删除黄金后重归一为66.67/16.67/16.67",
    ...
    snapshot: dict[str, Any] = {
        "schema_version": SCHEMA_V57,
        "engine_version": ENGINE_V57,
        ...
        "policy_benchmark": {
            "id": "policy_no_gold_66_17_17",
            "weights_internal_equity_bond_commodity": POLICY.tolist(),
            "display_cn": "删除黄金后三资产政策锚：权益66.67% + 国债16.67% + 商品16.67%",
            "optimizer_anchor_for_relative_models": True,
            "derivation": "原60/15/15/10政策基准删除黄金10%后，对权益/国债/商品剩余90%按比例重归一",
        },
        "display_benchmark": {
        ...
        "recommended": {
            ...
            "reason": "删除黄金后，风险平价仍是全区间Sharpe冠军和最低回撤方案；宏观因子是普林格第五阶段滞涨下的商品占优影子方案。",
        ...
        "benchmarks": {
            ...
            "policy_no_gold_66_17_17": _strategy_payload(
                "policy_no_gold_66_17_17",
                "三资产政策锚",
                policy_rows,
                equal_rows,
                {asset: float(POLICY[index]) for index, asset in enumerate(ASSET_ORDER)},
                role="relative optimizer anchor",
                construction=["原60/15/15/10删除黄金后重归一", "权益/国债/商品=66.67/16.67/16.67"],
