...
from legacy_b06_direct_v549 import direct_active_alpha_v549, legacy_b06_target_v549


def allocate_relative_legacy_direct_v549(
    ...
    *,
    ...
    if raw_returns.ndim != 2 or raw_returns.shape != (36, 4) or not np.all(np.isfinite(raw_returns)):
        raise ValueError("v549_direct_stack_requires_exact_36x4_window")
    validated_months = _validate_months(months, 36)
    if truth._production_cycles(current_cycle):
        raise RuntimeError("v549_production_cycle_history_pipeline_not_certified")
    ...
    signal_target, signal = legacy_b06_target_v549(
    ...
    if abs(float(parameters.active_risk_aversion) - float(parameters.risk_aversion)) > 1e-12:
        raise ValueError("v549_direct_delta_must_equal_optimizer_risk_aversion")
    ...
    if optimizer.get("status") != "optimal" or optimizer.get("weights") is None:
        raise RuntimeError("v549_direct_optimizer_not_optimal")
    residual = np.max(np.abs(alpha - parameters.active_risk_aversion * covariance @ (signal_target - POLICY_WEIGHTS_V541)))
    if residual > 1e-12:
        raise AssertionError("v549_direct_alpha_identity_failed")
    return {
...
__all__ = ["allocate_relative_legacy_direct_v549"]