...
def test_direct_stack_has_complete_kkt_and_no_other_inference():
    ...
    result=allocate_relative_legacy_direct_v556(returns,np.zeros_like(returns),np.zeros_like(returns,dtype=bool),_months(),{"cycles":{}},[.60,.15,.10,.15],parameters)
...
def test_direct_stack_macro_false_cells_and_future_are_inert():
    ...
    base=allocate_relative_legacy_direct_v556(returns,macro,admission,_months(),{"cycles":{}},[.60,.15,.10,.15],p)
    changed=macro.copy(); changed[:]=1e9
    counter=allocate_relative_legacy_direct_v556(returns,changed,admission,_months(),{"cycles":{}},[.60,.15,.10,.15],p)
...
def test_direct_stack_rejects_noncontiguous_calendar_and_production_cycle():
    ...
    with pytest.raises(ValueError,match="contiguous"):
        allocate_relative_legacy_direct_v556(r,z,z.astype(bool),months,{"cycles":{}},[.60,.15,.10,.15],p)
    ...
    with pytest.raises(RuntimeError,match="not_certified"):
        allocate_relative_legacy_direct_v556(r,z,z.astype(bool),_months(),cycle,[.60,.15,.10,.15],p)