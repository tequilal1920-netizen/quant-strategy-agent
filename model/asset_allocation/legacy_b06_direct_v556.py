...
ASSET_ORDER_V556 = ("equity", "bond", "gold", "commodity")
POLICY_WEIGHTS_V556 = np.asarray([.60, .15, .10, .15])
LOWER_BOUNDS_V556 = np.asarray([.10, .05, .05, .05])
UPPER_BOUNDS_V556 = np.asarray([.75, .40, .30, .40])
HORIZONS_V556 = (1, 3, 6)
HORIZON_WEIGHTS_V556 = (.15, .35, .50)
PROBABILITY_POWER_V556 = 2.0
PROBABILITY_SLOPE_V556 = 1.7
RELATIVE_STRENGTH_V556 = 1.0
VOLATILITY_PENALTY_V556 = .25
CORRELATION_PENALTY_V556 = .20
POLICY_ANCHOR_V556 = .10
STABILITY_BASE_V556 = .05
STABILITY_MAX_V556 = .50
STABILITY_CENTER_V556 = .50
STABILITY_SLOPE_V556 = 10.0
EQUITY_GUARD_MAX_V556 = .20
EQUITY_GUARD_CENTER_V556 = .55
EQUITY_GUARD_SLOPE_V556 = 10.0
ACTIVE_VOLATILITY_CONFIDENCE_TARGET_V556 = .08
MINIMUM_CONFIDENCE_KAPPA_V556 = .25
...
def frozen_spec_v556() -> dict[str, Any]:
    return {
        "asset_order": list(ASSET_ORDER_V556),
        "policy_weights": POLICY_WEIGHTS_V556.tolist(),
        "lower_bounds": LOWER_BOUNDS_V556.tolist(),
        "upper_bounds": UPPER_BOUNDS_V556.tolist(),
        "horizons": list(HORIZONS_V556),
        "horizon_weights": list(HORIZON_WEIGHTS_V556),
        "probability_power": PROBABILITY_POWER_V556,
        "probability_slope": PROBABILITY_SLOPE_V556,
        "relative_strength": RELATIVE_STRENGTH_V556,
        "volatility_penalty": VOLATILITY_PENALTY_V556,
        "correlation_penalty": CORRELATION_PENALTY_V556,
        "policy_anchor": POLICY_ANCHOR_V556,
        "stability": [STABILITY_BASE_V556, STABILITY_MAX_V556, STABILITY_CENTER_V556, STABILITY_SLOPE_V556],
        "equity_guard": [EQUITY_GUARD_MAX_V556, EQUITY_GUARD_CENTER_V556, EQUITY_GUARD_SLOPE_V556],
        "guard_redistribution": [-1.0, .60, .40, 0.0],
        "active_volatility_confidence_target": ACTIVE_VOLATILITY_CONFIDENCE_TARGET_V556,
        "minimum_confidence_kappa": MINIMUM_CONFIDENCE_KAPPA_V556,
        ...
        "annual_volatility_formula": "sqrt_diagonal_of_annualized_covariance_without_second_annualization",
...
EXPECTED_SPEC_SHA256_V556 = "D08A27C076B20041090C0BFCC0AAF4E80E3D1E9905C5BEE2D7E68CB01F191BFB"
SPEC_SHA256_V556 = _canonical_hash(frozen_spec_v556())
if SPEC_SHA256_V556 != EXPECTED_SPEC_SHA256_V556:
    raise RuntimeError("v556_release_signal_spec_hash_drift")


def _vector(values: Sequence[float], name: str) -> np.ndarray:
    result = np.asarray(values, dtype=float)
    if result.shape != (4,) or not np.all(np.isfinite(result)):
        raise ValueError(f"v556_{name}_invalid")
    return result


def bounded_simplex_v556(values: Sequence[float], lower_bounds: Sequence[float], upper_bounds: Sequence[float]) -> np.ndarray:
    ...
    if np.any(lower < 0.0) or np.any(upper < lower) or lower.sum() > 1.0 + 1e-12 or upper.sum() < 1.0 - 1e-12:
        raise ValueError("v556_bounds_infeasible")
    ...
    if abs(gap) > 1e-12:
        ...
        if float(room.sum()) <= 1e-15:
            raise RuntimeError("v556_projection_no_room")
    ...
    if abs(float(projected.sum()) - 1.0) > 1e-10 or np.any(projected < lower - 1e-10) or np.any(projected > upper + 1e-10):
        raise RuntimeError("v556_projection_failed")
    return projected
...
def legacy_b06_target_v556(return_history: np.ndarray, policy_weights: Sequence[float] = POLICY_WEIGHTS_V556, lower_bounds: Sequence[float] = LOWER_BOUNDS_V556, upper_bounds: Sequence[float] = UPPER_BOUNDS_V556) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    if history.ndim != 2 or history.shape[1] != 4 or len(history) < 36 or not np.all(np.isfinite(history)) or np.any(history <= -1.0):
        raise ValueError("v556_requires_36x4_finite_causal_history")
    ...
    if abs(float(policy.sum()) - 1.0) > 1e-12 or np.any(policy < lower) or np.any(policy > upper):
        raise ValueError("v556_policy_not_feasible")
    covariance12, diagnostics12 = _risk_covariance(history, 12)
    covariance36, diagnostics36 = _risk_covariance(history, 36)
    # robust_covariance already returns an annualized matrix because annualization=12.0.
    annual_volatility = np.maximum(np.sqrt(np.diag(covariance12)), .02)
    probability = np.zeros(4); risk_adjusted = np.zeros(4); horizon_probability = {}
    for horizon, coefficient in zip(HORIZONS_V556, HORIZON_WEIGHTS_V556):
        ...
        posterior = 1.0 / (1.0 + np.exp(-PROBABILITY_SLOPE_V556 * np.clip(standardized, -5.0, 5.0)))
    ...
    log_score = (
        PROBABILITY_POWER_V556 * np.log(np.maximum(probability, 1e-4))
        + RELATIVE_STRENGTH_V556 * np.tanh(_robust_cross_z(risk_adjusted) / 2.0)
        - VOLATILITY_PENALTY_V556 * np.tanh(_robust_cross_z(np.log(annual_volatility)) / 2.0)
        - CORRELATION_PENALTY_V556 * np.tanh(_robust_cross_z(average_correlation) / 2.0)
    )
    raw = np.exp(log_score - float(np.max(log_score)))
    tactical = raw / raw.sum()
    policy_anchored = POLICY_ANCHOR_V556 * policy + (1.0 - POLICY_ANCHOR_V556) * tactical
    inverse_volatility = 1.0 / np.sqrt(np.maximum(np.diag(covariance12), 1.0e-16))
    inverse_volatility /= float(inverse_volatility.sum())
    stability = bounded_simplex_v556(inverse_volatility, lower, upper)
    breadth = float(probability.mean())
    stability_weight = float(np.clip(STABILITY_BASE_V556 + STABILITY_MAX_V556 / (1.0 + math.exp(STABILITY_SLOPE_V556 * (breadth - STABILITY_CENTER_V556))), 0.0, .75))
    pre_guard = (1.0 - stability_weight) * policy_anchored + stability_weight * stability
    guard = min(EQUITY_GUARD_MAX_V556 / (1.0 + math.exp(EQUITY_GUARD_SLOPE_V556 * (float(probability[0]) - EQUITY_GUARD_CENTER_V556))), max(float(pre_guard[0] - lower[0]), 0.0))
    ...
    kappa = float(np.clip(ACTIVE_VOLATILITY_CONFIDENCE_TARGET_V556 / max(worst, 1e-8), MINIMUM_CONFIDENCE_KAPPA_V556, 1.0))
    target = bounded_simplex_v556(policy + kappa * (post_guard - policy), lower, upper)
    return target, {
        "status": "legacy_transfer_challenger_not_blind_champion", "asset_order": list(ASSET_ORDER_V556), "spec_sha256": SPEC_SHA256_V556,
        ...
        "raw_tactical_score": raw.tolist(), "tactical_weights": tactical.tolist(), "policy_anchor_weight": POLICY_ANCHOR_V556, "policy_anchored_weights": policy_anchored.tolist(), "stability_sleeve": stability.tolist(), "breadth": breadth, "stability_weight": stability_weight,
        ...
        "active_volatility_12m": active_vol12, "active_volatility_36m": active_vol36, "active_volatility_confidence_target": ACTIVE_VOLATILITY_CONFIDENCE_TARGET_V556, "kappa": kappa,
...
def direct_active_alpha_v556(covariance: Sequence[Sequence[float]], signal_target: Sequence[float], policy_weights: Sequence[float] = POLICY_WEIGHTS_V556, active_risk_aversion: float = 4.0) -> np.ndarray:
    ...
    if matrix.shape != (4, 4) or not np.all(np.isfinite(matrix)) or np.linalg.norm(matrix - matrix.T, ord="fro") > 1e-10 * max(np.linalg.norm(matrix, ord="fro"), 1e-15):
        raise ValueError("v556_covariance_invalid")
    delta = float(active_risk_aversion)
    if not math.isfinite(delta) or delta <= 0.0 or abs(float(target.sum()) - 1.0) > 1e-10:
        raise ValueError("v556_direct_alpha_input_invalid")
    return delta * matrix @ (target - policy)


__all__ = ["SPEC_SHA256_V556", "bounded_simplex_v556", "direct_active_alpha_v556", "frozen_spec_v556", "legacy_b06_target_v556"]