...
from backtest_asset_allocation_v551_long import (
    GRID_HASH_V551,
    PRETEST_YEARS_V551,
    TEST_START_V551,
    _canonical_hash,
...
def panel() -> dict:
    ...
    payload = {
        "schema_version": "asset-allocation-panel-v550-T2-signal-self-financing-d2-research/1.0",
...
def row(mode: str, *, good: bool = True) -> dict:
    ...
    return {
        ...
        "pretest_calendar_years": {year: dict(annual) for year in PRETEST_YEARS_V551},
    }


def test_grid_is_exactly_eight_and_hash_locked():
    grid = candidate_grid_v551()
    assert [item["id"] for item in grid] == [
        "V551-REL-01", "V551-REL-02", "V551-REL-03", "V551-REL-04",
        "V551-ABS-01", "V551-ABS-02", "V551-ABS-03", "V551-ABS-04",
    ]
    assert _canonical_hash(grid) == GRID_HASH_V551
...
def test_pretest_simulator_never_receives_test_months():
    ...
    assert max(months) < TEST_START_V551
...
def test_selector_rejects_wrong_years_or_test_metrics():
    ...
    with pytest.raises(ValueError, match="calendar_boundary"):
        select_pretest_v551([changed], "benchmark_relative")
    changed = copy.deepcopy(candidate)
...
def test_absolute_selection_uses_own_sharpe_not_policy_excess():
    ...
    selected, board = select_pretest_v551([candidate], "absolute_no_benchmark")
...
def test_relative_selection_still_requires_positive_active_evidence():
    selected, board = select_pretest_v551([row("benchmark_relative", good=False)], "benchmark_relative")
...
def test_test_counterfactual_cannot_change_pretest_selected_or_board():
    original = panel()
    base_result = build_research_v551(original)
    ...
    changed["content_sha256"] = _canonical_hash({key: value for key, value in changed.items() if key != "content_sha256"})
    counter = build_research_v551(changed)
...
def test_result_is_byte_reproducible_and_equal_weight_absent():
    first = build_research_v551(panel())
    second = build_research_v551(panel())
    assert first == second
