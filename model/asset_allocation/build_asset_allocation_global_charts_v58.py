"""Build reference-style PNG charts for the v5.8 no-gold asset-allocation block."""
...
OUTPUT_DIR = PROJECT_ROOT / "output" / "asset_allocation_global_charts_v58_no_gold"
EXPECTED_SCHEMA = "5.8.0"
EXPECTED_SNAPSHOT_HASH = "F787AE0F1600306B184A8F778DD0B9B299CFB9397CDBA6D9EA796F5CCF9C2FC7"


def _read_snapshot() -> dict[str, Any]:
    data = json.loads(SNAPSHOT_PATH.read_text(encoding="utf-8"))
    if data.get("schema_version") != EXPECTED_SCHEMA:
        raise ValueError("asset_allocation_v58_snapshot_schema_mismatch")
    if data.get("content_sha256") != EXPECTED_SNAPSHOT_HASH:
        raise ValueError("asset_allocation_v58_snapshot_hash_mismatch")
    if data.get("asset_order") != ["equity", "bond", "commodity"]:
        raise ValueError("asset_allocation_v58_asset_order_mismatch")
    return data
...
def build() -> dict[str, Any]:
    ...
    strategies = [
        ...
        ("02_risk_parity", "风险平价", models["risk_parity"]),
        ("03_all_weather", "全天候", models["all_weather"]),
        ("04_macro_factor", "宏观因子", models["macro_factor"]),
    ]
    ...
    audit: dict[str, Any] = {
        "schema_version": "asset-allocation-global-performance-charts-v58-no-gold/1.0",
