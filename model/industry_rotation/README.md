...
- `style_box_rotation.py`：季度股票级 3×4 风格域标签 + 月度12风格域六维轮动入口；旧季度Top3模型保留在快照的 `legacy_quarterly`，不再作为可见主策略。
...
行业侧固定为31个申万一级行业、每行业8个互异专属业务字段。月度和周度候选在预声明的Top10/Top5、等权/风险权重、持仓缓冲和现金预算结构内比较，并以同频31行业等权为基准。风格侧在每个季度末将全部合格A股唯一标记为“大/中/小盘 × 成长/均衡/价值/红利”12个互斥且穷尽的风格域；每个月末沿用最近季度标签，把股票级六维因子聚合成12个风格域得分，下一交易日执行月度Top2风格域组合，并以12风格域月度等权为基准。训练、验证、测试严格分离，测试集只报告或否决唯一挑战者，不参与候选排序。
## 回测复现与审计口径
