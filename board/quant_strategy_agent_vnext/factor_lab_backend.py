...
def governed_history(rows: list[sqlite3.Row]) -> list[dict[str, Any]]:
    """Keep one completed audit record per model family without using test metrics."""

    mode_rank = {"production": 3, "research": 2, "smoke": 1}
    ...
    for row in rows:
        ...
        score = (
            mode_rank.get(str(item.get("mode") or "").lower(), 0),
            str(item.get("completed_at") or item.get("created_at") or ""),
            str(item.get("run_id") or ""),
        )
        if current is None or score > current["_governance_score"]:
            item["selection_basis"] = "completed_model_family_record_without_test_ranking"
            item["selection_uses_test"] = False
            item["_governance_score"] = score
            selected[engine] = item
    output = []
    for item in selected.values():
        clean = dict(item)
        clean.pop("_governance_score", None)
        output.append(clean)
...
def register_factor_lab(app: Flask) -> None:
    ...
    def list_runs():
        ...
        return jsonify({
            "status": "ok",
            "runs": [run_dict(row) for row in rows],
            "governed_history": governed_history(list(rows)),
            "history_policy": {
                "keep": "one_completed_record_per_model_family",
                "selection_uses_test": False,
                "raw_audit_retained_server_side": True,
            },
        })
