"""Graph-first visual adapter for the v5.7 no-gold asset-allocation snapshot."""
...
COLORS = {
    ...
    "policy_no_gold_66_17_17": "#163d7a",
}
...
def _asset_order(data: Mapping[str, Any]) -> list[str]:
    ...
    if order != ["equity", "bond", "commodity"]:
        raise ValueError("asset_allocation_v57_visual_requires_three_assets_no_gold")
    return order
...
def build(data: dict[str, Any], metrics: list[dict[str, Any]] | None = None, page: str = "strategy") -> dict[str, Any]:
    ...
    policy = benchmarks.get("policy_no_gold_66_17_17") or {}
    ...
    diagnostics = {
        ...
        "secondary_charts": [
            {
                "title": "相对三资产政策锚66.67/16.67/16.67的高低配",
                "x_title": "资产",
                "y_title": "主动权重",
                "traces": [
                    _trace(
    ...
    strategy = {
        "title": "最终策略：Sharpe冠军与滞涨周期一致性双口径展示（三资产无黄金）",
        "display": "charts_only",
        "table": _table(
            [
                ("model", "模型", "text"),
        ...
        "secondary_charts": [
            {
                ...
                "y_title": "权重",
                "traces": [
                    _trace(str(recommended_model.get("name") or "Sharpe冠军"), asset_names, _weights(data, recommended_model), color=COLORS.get(recommended_key, "#7f7f7f"), kind="bar"),
                    _trace("宏观因子（普林格五）", asset_names, _weights(data, macro_model), color=COLORS["macro_factor"], kind="bar"),
                    _trace("政策锚66.67/16.67/16.67", asset_names, _weights(data, policy), color=COLORS["policy_no_gold_66_17_17"], kind="bar"),
                ],
            },
            {
