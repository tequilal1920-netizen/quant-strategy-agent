  ...
  for (const item of targets) {
    ...
    try {
      ...
      for (const section of sections) {
        ...
        await page.locator(`[data-workspace-section="${section}"]`).click({ timeout: 10_000 });
        await page.waitForFunction(({ target, section }) => {
          const button = document.querySelector(`[data-workspace-section="${section}"]`);
          const root = document.querySelector("#view-root");
          const shadow = root?.querySelector("#ai-monitor-host")?.shadowRoot;
          const text = `${root?.innerText || ""} ${shadow?.textContent || ""}`.trim();
          const sectionReady = target !== "data:ai_monitor" || Boolean(shadow?.querySelector(`#${CSS.escape(section)}`));
          return Boolean(button?.classList.contains("is-active") && text.length > 20 && sectionReady);
        }, { target: item.target, section }, { timeout: item.target === "data:ai_monitor" ? 30_000 : 30_000 });
