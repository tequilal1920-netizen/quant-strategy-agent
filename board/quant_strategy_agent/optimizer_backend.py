...
class OptimizerBackendService:
    ...
    def strategy_snapshot(self) -> dict[str, Any]:
        ...
        try:
            with sqlite3.connect(
                ...
                latest = connection.execute(
                    ...
                    ORDER BY signal_date DESC, MAX(rowid) DESC
                    LIMIT 1
                    """
                ).fetchone()
                if latest is None:
        ...
        framework = {
            ...
            "module_linkage": [
                {"source": "?????", "target": "??Alpha??", "field": score_meta.get("score_name")},
                {"source": "????", "target": "??Alpha?????", "field": "industry_rotation_v4"},
                {"source": "????", "target": "???/????", "field": "point-in-time returns"},
                {"source": "????", "target": "???????", "field": "previous optimized weights"},
                {"source": "????", "target": "??????", "field": latest_timing.get("active_side")},
                {"source": "?????", "target": "??50???", "field": "certified target weights"},
            ],
            "factor_model": {
