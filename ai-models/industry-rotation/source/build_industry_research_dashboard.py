...
import engine as rotation_engine
...
def _factor_quality_score(row: dict[str, Any], bad_signal: bool = False) -> float | None:
    coverage = _row_float(row, "coverage")
    train_ic = _row_float(row, "train_ic")
    valid_ic = _row_float(row, "valid_ic")
    train_icir = _row_float(row, "train_icir")
    valid_icir = _row_float(row, "valid_icir")
    spread = _row_float(row, "valid_spread")
    positive = _row_float(row, "valid_positive", 0.5)
    sign = -1.0 if bad_signal else 1.0
    if coverage < 0.60:
        return None
    if sign * train_ic <= 0.0 or sign * valid_ic <= 0.0 or sign * spread <= 0.0:
        return None
    if (not bad_signal and positive < 0.50) or (bad_signal and positive > 0.50):
        return None
    icir_part = 0.25 * max(sign * train_icir, 0.0) + 0.35 * max(sign * valid_icir, 0.0)
    spread_part = 0.25 * min(max(sign * spread, 0.0), 0.18) / 0.18
    hit_part = 0.15 * max(sign * (positive - 0.50), 0.0) * 2.0
    return float(icir_part + spread_part + hit_part)


def _selected_factor_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    plan = [
        ("prosperity", False, 4),
        ("fundamental", False, 6),
        ("technical", False, 6),
    ...
    for dim, bad_signal, limit in plan:
        candidates: list[dict[str, Any]] = []
        for row in rows:
            if row.get("dimension") != dim:
                continue
            score = _factor_quality_score(row, bad_signal=bad_signal)
            if score is None:
                continue
            item = dict(row)
            item["selection_score"] = _finite(score)
            item["selection_role"] = "拥挤扣分" if bad_signal else "收益信号"
            candidates.append(item)
        output.extend(sorted(candidates, key=lambda r: r.get("selection_score") or -9, reverse=True)[:limit])
    return output


def _weighted_factor_cluster(
    ...
    for row in selected:
        ...
        if frame is None:
            continue
        weight = max(_row_float(row, "selection_score"), 0.01)
    ...
    if numerator is None or denominator is None:
        return fallback
    return _pct_rank(numerator.div(denominator.replace(0.0, np.nan)))


def _enhanced_rotation_score(
    factors: dict[str, pd.DataFrame],
    dimensions: dict[str, pd.DataFrame],
    factor_rows: list[dict[str, Any]],
) -> tuple[pd.DataFrame, dict[str, Any]]:
    selected = _selected_factor_rows(factor_rows)
    by_dim = {
        dim: [row for row in selected if row.get("dimension") == dim]
        for dim in ["prosperity", "fundamental", "technical", "valuation", "funds", "crowding"]
    }
    anchor = _c39_score(factors, dimensions)
    technical = _weighted_factor_cluster(factors, by_dim["technical"], dimensions["technical"])
    fundamental = _weighted_factor_cluster(factors, by_dim["fundamental"], dimensions["fundamental"])
    crowding = _weighted_factor_cluster(factors, by_dim["crowding"], dimensions["crowding"])
    raw = (
        anchor.sub(0.5)
        .mul(0.62)
        .add(technical.sub(0.5).mul(0.30), fill_value=np.nan)
        .add(fundamental.sub(0.5).mul(0.08), fill_value=np.nan)
        .add(crowding.sub(0.5).mul(-0.05), fill_value=np.nan)
    )
    score = _pct_rank(raw)
    return score, {
        "model_id": "C45_monthly_verified_quality_trend_crowding_top7_risk_weighted_buffered",
        "anchor": "C39景气盈利确认",
        "overlay": "训练验证双通过的技术趋势、基本面确认与拥挤扣分因子簇",
        "weights": {
            "C39景气盈利主锚": 0.62,
            "技术趋势有效簇": 0.30,
            "基本面确认有效簇": 0.08,
...
def _simulate_monthly_score(
    score: pd.DataFrame,
    close: pd.DataFrame,
) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    targets = rotation_engine._targets(
        score,
        "monthly",
        close=close,
        top_n=7,
    ...
    simulation, holdings = rotation_engine._simulate(close, targets, cost_rate=0.001)
    metrics = rotation_engine._all_metrics(simulation)
    nav = [
        {
            "date": date.strftime("%Y-%m-%d"),
            "strategy": _finite(row.nav),
            "benchmark": _finite(row.benchmark_nav),
            "excess": _finite(row.nav / row.benchmark_nav),
        }
        for date, row in simulation.iterrows()
    ]
...
def _efficient_factor_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    selected = _selected_factor_rows(rows)
    if selected:
        return selected
    output: list[dict[str, Any]] = []
    for dim in ["prosperity", "fundamental", "technical", "valuation", "funds", "crowding"]:
        sub = [dict(r) for r in rows if r.get("dimension") == dim]
        for row in sub:
...
def _build_rotation_section(
    ...
    six = best_snapshot.get("six_dimension") or snapshot.get("six_dimension") or {}
    factor_rows = _factor_rows(six)
    display_factor_rows = _factor_rows(snapshot.get("six_dimension") or six)
    ...
    metrics, holdings, nav = _simulate_monthly_score(score, close)
    latest_holding = (holdings or [{}])[-1]
    ...
    annual_attr = _attribution_rows(holdings, daily_ret, score, dimensions, "year")
    monthly_attr = _attribution_rows(holdings, daily_ret, score, dimensions, "month")
    ...
    return {
        ...
        "factor_table": display_factor_rows,
...
def build_payload() -> dict[str, Any]:
    ...
    payload = {
        ...
        "rotation": _build_rotation_section(snapshot, best_snapshot, final_figures, factors, dimensions, daily_ret, _close),
    }
    return payload
