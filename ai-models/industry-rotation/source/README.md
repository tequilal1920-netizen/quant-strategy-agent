...
- `style_six_dimension_monthly.py`：股票级标签聚合为12格风格箱、3类市值、4类风格，沿用景气度、基本面、技术面、估值、资金面、拥挤度六维框架，月末信号、下一交易日执行。
- `build_rotation_final_figures.py`：从正式 JSON 生成行业轮动和三套风格轮动的年度收益表与日频净值图，写入 vNext 网页静态目录。

治理口径：训练集和验证集用于因子方向、因子准入和候选选择；测试集只报告或否决。网页展示只读取脱敏图表清单，不暴露完整股票级和因子级明细。
